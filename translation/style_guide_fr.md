...
1. Traduire uniquement la colonne `fr` du catalogue UTF-8 et passer le statut à `traduit`.
   **L'absence de terme anglais dans la cible ne suffit pas. Tout terme canonique identifié dans la source doit correspondre à son équivalent français canonique dans la cible.**
   Pour une capacité anglaise monosémique ambiguë dans son contexte (`Thunder`, par exemple), inscrire son identifiant dans `canonical_ids`. Plusieurs IDs sont séparés par `;`. Dès qu'un ID est annoté, son `french_canonical` exact devient obligatoire.
