...
FIELDS = ["id", "file", "line", "label", "encoding", "source", "fr", "canonical_ids", "status", "notes"]
...
def main() -> int:
    ...
    for item in iter_strings(args.root):
        ...
        item.update(fr=old.get("fr", ""), canonical_ids=old.get("canonical_ids", ""),
                    status=old.get("status", "à_traduire"), notes=old.get("notes", ""))
        rows.append(item)
